...
FeatureNames = {'Envelope', 'Phoneme Onset'};
...
for cond = 1: NoConditions
    ...
    for f=1:size(model_Fwd_s_cond{1},2)
        ...
        t0=find(t_lag==0);
         ...
         for lagi = 1:length(twindStart)   
            ...
            lagwind=t0:t0+(twind-1);
                ...
                model_Fwd_acrlagwind = squeeze(mean(model_Fwd_s_cond{cond}(:,f,lagwind,:),3)); %averaged across lag within the window         
         ...
         sgtitle(sprintf(['TRF ', FeatureNames{f}]));

         baseFigNameh8 = sprintf(['MultiModel_GA_feature',FeatureNames{f},'_TRF_Topo', '_N' num2str(nsubj), '_', CondName{cond},'.fig']);
