...
% 1. Load normalized Audio envelope and normalized EEG trials
...
for subj=1:length(Subjects)
     ...
     EEGs=eeg.data;
        ...
        r_mean_SpFeature_subj(subj,:) = squeeze(mean(r_mean_SpFeature,3)); %avg across electrodes
            ...
            baseFigName = sprintf(['%s_crossval_lambda_Sp_Uni_cond_',CondName{1}, '_' ,num2str(MinTlag), '_', num2str(MaxTlag), '.fig'],num2str(Subjects{subj}));
...
clear eeg EEGs AudioEnvSp_rescale AudioFeatureSp
...
save([PATHOUT,'LambdaTest_Uni' 'N' num2str(length(Subjects)) '_SpFeature', num2str(MinTlag), '_', num2str(MaxTlag), '.mat'], 'mse_mean_SpFeature_subj','r_mean_SpFeature_subj','lambdas') 
disp ('Done!')
